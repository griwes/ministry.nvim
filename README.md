...
- editor tools are identifier-first:
  ...
  - `apply_diff_*` tools accept explicit surgical `hunks` generated by the caller; `diff_*` tools compute hunks from full target content when that is useful, while `write_*` tools perform exact full-buffer replacement
